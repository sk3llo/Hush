...
class AppPreferences {
    ...
    private func registerDefaults() {
        let defaultValues: [String: Any] = [
            ...
            Keys.model: "gemini-2.5-flash", // Default to 2.5 version
    ...
    /// Get/set the OpenAI API key (stored in keychain)
    var openAIApiKey: String? {
        get {
            return getOpenAIApiKeyFromKeychain()
        }
        set {
            if let newValue = newValue {
                saveOpenAIApiKeyToKeychain(newValue)
            } else {
                deleteOpenAIApiKeyFromKeychain()
            }
        }
    }
    ...
    var model: String {
        get { defaults.string(forKey: Keys.model) ?? "gemini-2.5-flash" }
...
//        saveChatsLocally = true
    ...
    /// Save OpenAI API key to the keychain
    private func saveOpenAIApiKeyToKeychain(_ apiKey: String) {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: Keys.keychainService,
            kSecAttrAccount as String: Keys.chatGPTApiKey,
            kSecValueData as String: apiKey.data(using: .utf8)!,
            kSecAttrAccessible as String: kSecAttrAccessibleAfterFirstUnlock
        ]
    ...
    private func getOpenAIApiKeyFromKeychain() -> String? {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: Keys.keychainService,
            kSecAttrAccount as String: Keys.chatGPTApiKey,
    ...
    private func deleteOpenAIApiKeyFromKeychain() {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: Keys.keychainService,
            kSecAttrAccount as String: Keys.chatGPTApiKey
