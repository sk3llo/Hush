...
struct SettingsView: View {
    ...
    /// State for Open AI paste feedback
    @State private var showOpenAIPasteFeedback = false
    
    /// State for API key visibility
    @State private var showAPIKey = false
    
    /// State for Open AI API key visibility
            ...
            Text("API Configuration")
                ...
                SettingsSection(title: "Google Gemini") {
                    ...
                    .foregroundColor(.secondary)
                            ...
                            HStack(spacing: 8) {
                                if showOpenAIAPIKey {
                                    TextField("Enter your Open AI API key", text: $viewModel.openAIApiKey)
                                        .textFieldStyle(.roundedBorder)
                                } else {
                                    SecureField("Enter your Open AI API key", text: $viewModel.openAIApiKey)
                                        .textFieldStyle(.roundedBorder)
                                }
                    ...
                    .foregroundColor(.secondary)
                                ...
                                .buttonStyle(.bordered)
                                .help(showOpenAIAPIKey ? "Hide Open AI API key" : "Show API key")
                            }
                            ...
                            HStack {
                                if viewModel.showOpenAIApiKeySaved {
                                    HStack(spacing: 4) {
                                        Image(systemName: "checkmark.circle.fill")
                                            .foregroundColor(.green)
                                        Text("Saved")
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Picker("Model", selection: $viewModel.model) {
                                ...
                                Text("GPT 4.0 Mini").tag("gpt-4o-mini")
            ...
            Text("Keyboard Shortcuts")
                ...
                SettingsSection(title: "Available Shortcuts") {
                    LazyVStack(spacing: 8) {
                        ...
                        ShortcutRow(key: Constants.SettingKeys.Shortcuts.process, title: "Process", shortcut: "⌘⇧S", viewModel: viewModel)
    ...
    /// Pastes OpenAI API key from clipboard
    private func pasteOpenAIAPIKey() {
        let pasteboard = NSPasteboard.general
        if let clipboardContent = pasteboard.string(forType: .string), !clipboardContent.isEmpty {
            viewModel.openAIApiKey = clipboardContent.trimmingCharacters(in: .whitespacesAndNewlines)
            
            // Show paste feedback
            showOpenAIPasteFeedback = true
